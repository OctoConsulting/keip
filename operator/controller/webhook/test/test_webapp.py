...
def test_sync_endpoint_success(test_client):
    ...
    request = request | {"controller": {}, "children": {}, "finalizing": False}
...
def test_customize_endpoint_success(test_client):
    request = load_json("json/customize-request.json")

    response = test_client.post("/customize", json=request)
    expected = load_json("json/customize-response.json")

    assert expected == response.json()
    assert response.status_code == 200


def test_customize_endpoint_invalid_json(test_client):
    response = test_client.post("/customize", json=None)

    assert response.status_code == 400


def test_customize_endpoint_empty_body(test_client):
    response = test_client.post("/customize", json={})
